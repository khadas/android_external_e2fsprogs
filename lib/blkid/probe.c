			  ...
			  const char *inbuf, int32_t inlen, char *outbuf, int32_t outlen)
{
    ...
    if (!initialized) {
        void *handle = dlopen("libicuuc_system.so", RTLD_LAZY);
        if (!handle)
            return -1;

        setenv("ICU_DATA", "/system/usr/icu", 1);
        func = dlsym(handle, "ucnv_convert_66");
        if (!func)
            return -1;
		      ...
		      unsigned char *buf)
{
	...
	printf("-->serno= %s, label_len=%d, label = %s\n", serno, label_len, label);
    ...
    if (label != 0) {
        ...
        int ret = gbk2utf8(inbuf, inlen, outbuf, outlen);

        if (!ret) {
            label = outbuf;
            label_len = sizeof(outbuf) - outlen;
        }
        printf("HAVE_DLOPEN -->ret= %d, fat usb label = %s\n", ret, label);
    }
#endif
