...
void *volume_id_get_buffer(struct volume_id *id, uint64_t off, size_t len)
{
	...
	ssize_t read_len;
        if (!NoDbg)
	    fprintf(stderr, "util, get buffer off 0x%llx(%llu), len 0x%zx\n",
	...
	if (off + len <= SB_BUFFER_SIZE
	 /* && off <= SB_BUFFER_SIZE - want this paranoid overflow check? */
		...
		len += off;
                if (!NoDbg)
		    fprintf(stderr, "util, len:0x%x, sbbuf_len:0x%x\n", (unsigned) len, (unsigned)id->sbbuf_len);
		if (len <= id->sbbuf_len)
			goto ret; /* we already have it */

                if (!NoDbg)
		    fprintf(stderr, "util, read sbbuf len:0x%x\n", (unsigned) len);
		id->sbbuf_len = len;
		off = 0;
		goto do_read;
	}
	...
	if (len > SEEK_BUFFER_SIZE) {
                if (!NoDbg)
		    fprintf(stderr, "util, seek buffer too small %d\n", SEEK_BUFFER_SIZE);
		return NULL;
	}
	dst = id->seekbuf;
	...
	dst = id->seekbuf;
        if (!NoDbg)
	    fprintf(stderr, "util, read seekbuf off:0x%llx len:0x%zx\n",
				(unsigned long long) off, len);
 do_read:
	if (lseek(id->fd, off, SEEK_SET) != (int64_t) off) {
		fprintf(stderr, "util, seek(0x%llx) failed\n", (unsigned long long) off);
		goto err;
	}
	read_len = full_read(id->fd, dst, len);
	if (read_len != (ssize_t) len) {
		fprintf(stderr, "util, requested 0x%x bytes, got 0x%x bytes\n",
 ...
 err:
		 ...
		 */
                fprintf(stderr, "util, now enter err, off:%llx\n", (uint64_t)off);
		if (off < 64*1024)
			id->error = 1;
