...
{
	struct exfat_super_block *sb = NULL;
	...
	// Primary super block
        if (!NoDbg)
	    fprintf(stderr, "exFAT: probing at offset 0x%llx, sb size:0x%zx\n", (uint64_t)EXFAT_SB_OFFSET, (size_t)sizeof(*sb));
	...
	if (memcmp(sb->fs_name, "EXFAT   ", 8) != 0)
		return -1;
        if (!NoDbg)
            fprintf(stderr, "exfat, now before memcmp, fs_name:%s, isLE:%d\n", sb->fs_name, is_little_endian());

	sector_sz = 1 << sb->bytes_per_sector;
	cluster_sz = sector_sz << sb->sectors_per_cluster;
        if (!NoDbg)
            fprintf(stderr, "exfat, heap_offset:0x%llx, root_dir:0x%llx \n", (uint64_t)sb->cluster_heap_offset, (uint64_t) sb->root_dir);
	...
	root_dir_off = (uint64_t)EXFAT_SB_OFFSET +
		...
		(sb->cluster_heap_offset) * sector_sz +
		(sb->root_dir - 2) * cluster_sz;
		//(le32_to_cpu(sb->cluster_heap_offset)) * sector_sz +
		//(le32_to_cpu(sb->root_dir) - 2) * cluster_sz;
        if (!NoDbg) {
	    fprintf(stderr, "exFAT: sector size 0x%x bytes\n", sector_sz);
	    fprintf(stderr, "exFAT: cluster size 0x%x bytes\n", cluster_sz);
	    fprintf(stderr, "exFAT: root dir is at 0x%llx\n", (long long)root_dir_off);
        }
	...
	for (count = 0; count < EXFAT_MAX_DIR_ENTRIES; count++) {
		...
		if (de->entry_type == 0x00) {
			// End of Directory Marker
                        if (!NoDbg)
		            fprintf(stderr, "exFAT: End of root directory reached after %u entries", count);
			break;
		}
